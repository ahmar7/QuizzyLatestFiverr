...
function Create(props) {
  ...
  const [lastDraftSavedTime, setLastDraftSavedTime] = useState(Date.now());
  const saveDelay = 1000; // Delay between draft saves in miliseconds
  const [savedDraftText, setSavedDraftText] = useState("Draft saved ");
  const [drafts, setDrafts] = useState([]);

  const [lastSetChanged, setLastSetChanged] = useState(null);
  ...
  // Title state
  const [titleValue, setTitleValue] = useState(""); // mainly used for drafts
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      if (lastDraftSavedTime) {
        setSavedDraftText("Draft saved " + timeSince(lastDraftSavedTime));
      }
    }, 1000); // You can adjust the interval here
      ...
      <section className="create">
        {errorFound && (
          <FloatingPopup
            ...
            icon="alert"
            enabled={errorFound}
            setEnabled={setErrorFound}
          ></FloatingPopup>
        )}
        {/* Drafts */}
        <h1 className="title">
          Previous Drafts
          <div style={{ marginTop: "20px" }} className="drafts">
            {drafts.map((draftData) => (
              <div className="set-info">
                <div className="set-icon">
                  ...
                  >
                    <path
                      ...
                      d="M9,39H29V29H39V9H9Zm0,3a2.988,2.988,0,0,1-3-3V9A2.988,2.988,0,0,1,9,6H39a2.988,2.988,0,0,1,3,3V30L30,42Zm6-15V24h8.5v3Zm0-8V16H33v3ZM9,39v0Z"
                ...
                <div className="set-content">
                  <p>{draftData.title}</p>
                  <p className="desc">
                    {"Last edited " + draftData.last_edited}
                  </p>
                </div>
                <button
                  className="remove-container edit-container"
                  onClick={() => {}}
                >
                  <p>Edit</p>
                </button>
        ...
        </h1>
        {
          ...
          >
            {"Draft saved " + timeSince(lastDraftSavedTime)}
          </a>
        }
                        ...
                        >
                          ...
                          equationData[index][0] == true ? (
                            <EquationEditor
                              ...
                              onChange={(eq) => {
                                setEquationData((prevEquationData) => {
                                  const updatedEquationData = JSON.parse(
                                    JSON.stringify(prevEquationData)
                                  );
                                  updatedEquationData[index][1] = eq;
                                  return updatedEquationData;
                                });
                              }}
                            ...
                            <EquationEditor
                              ...
                              onChange={(eq) => {
                                setEquationData((prevEquationData) => {
                                  ...
                                  updatedEquationData[index][3] = eq;
                                  return updatedEquationData;
                                });
                              }}
                              autoCommands="bar overline sqrt sum prod int alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omikron pi rho sigma tau upsilon phi chi psi omega rangle langle otimes neq leq ll geq gg approx dagger angle and or infty"
