...
import {
  ...
  get_user,
  get_set_data,
  update_recall_session,
  get_recall_info,
  get_recall_intervals,
  get_recall_queue,
  submit_recall_choice,
  on_set_closed,
  update_recall_settings,
...
const RECALL_INDEXES_LEARNING = ["again", "good", "perfect"];

const RECALL_INDEXES = ["again", "hard", "good", "perfect"];
function Recall(props) {
  ...
  const [answeredDefinition, setAnsweredDefinition] = useState("");
  const [equationEditorIsReadOnly, setEquationEditorIsReadOnly] =
    useState(false);

  const [expectedTitle, setExpectedTitle] = useState(null);
  const [currentTermPositionInQueue, setCurrentTermPositionInQueue] =
    useState(0);
  const [oftenSliderValue, setOftenSliderValue] = useState(0);
  const [oftenSliderLabel, setOftenSliderLabel] = useState("Not very often");

  const [currentTermStamp, setCurrentTermStamp] = useState(
    "/images/recall/extremely_dissatisfied.svg"
  );
  ...
  const [bottomFlashcardInputDisabled, setBottomFlashcardInputDisabled] =
    useState(false);
  ...
  const [equationEditorKey, setEquationEditorKey] = useState(0);
  ...
  const animateBottomFlashcard = useSpring({
    from: {
      ...
      marginTop: bottomFlashcardShown ? -340 : -20,
    },
    to: {
      opacity: bottomFlashcardShown ? 1 : 0,
      marginTop: bottomFlashcardShown ? -20 : -340,
    },
  ...
  const animateBottomFlashcardTextArea = useSpring({
    from: {
      background: bottomFlashcardInputDisabled
        ? "white"
        : DISABLED_TEXT_AREA_COLOR,
    },
    to: {
      background: bottomFlashcardInputDisabled
        ? DISABLED_TEXT_AREA_COLOR
  ...
  const onFinishedTypingDefinition = function (typedDefinition) {
    ...
    if (bottomFlashcardInput && bottomFlashcardInput.current) {
      bottomFlashcardInput.current.readOnly = true;
    }
    setEquationEditorIsReadOnly(true);

    // Smart Completion
    if (recallSettings.smart_completion) {
      if (typedDefinition === def) {
        /* If what the user has typed is == the definition of the value.. */
        assumeAnswer(3);
      } else if (typedDefinition.length === 0) {
        /* If the user has typed nothing in, then they don't know it */
        assumeAnswer(0);
      } else {
        setEmojisShown(true);
      }
  ...
  const onEmojiClicked = function (emojiIndex) {
    ...
    submit_recall_choice(
      setid,
      recallIntervals.length === 3
        ? RECALL_INDEXES_LEARNING[emojiIndex]
        : RECALL_INDEXES[emojiIndex],
      answeredDefinition,
      function () {
        setEmojisShown(false);
        ...
        updateRecallQueue(function (finished, d) {
          if (!finished) {
            ...
            setTimeout(function () {
              ...
              setEquationEditorIsReadOnly(false);

              setAnsweredDefinition("");
              setEquationEditorKey((prevKey) => prevKey + 1);
              setTermStampShown(false);
              setTermShown(true);
              showBottomFlashCard(true);
              setIsBusy(false);
  ...
  useEffect(() => {
    if (
      currentTermPositionInQueue > 0 &&
      setData != undefined &&
      setData.terms.length > 0
    ) {
      setRecallPercentage(
        Math.floor((currentTermPositionInQueue / recallQueue.length) * 100)
      );
                ...
                <div className="container-top-radio">
                  ...
                  <div className="radio-container" ref={smartCompletionRadio}>
                    <div
                      ...
                      />
                      <label for="smart-completion-on">On</label>
                    </div>
                    <div
                      style={{
                        display: "flex",
                      ...
                      <input
                        ...
                        defaultChecked={!recallSettings["smart_completion"]}
                      />
                      <label for="smart-completion-off">Off</label>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bottom">
                  <p className="title">
                    {setData?.psd?.test?.test_date == undefined
                      ? "You have not entered a Test Date for this set."
                      : "Your Test Date is " +
                        stringifyDate(setData?.psd?.test?.test_date, true) +
                        "."}
                  </p>
                  <p className="sub-title">
                    {setData?.psd?.test?.test_date == undefined
                      ? "Quizzy will schedule Recall Sessions for this set to put you in the best spot to remember everything for the long-term."
                      : "Quizzy will schedule Recall Sessions for this set to put you in the best spot to get that A."}
                  </p>
                </div>
              </div>
              <div className="create-btn-cont">
                <button
                  ...
                  onClick={() => {
                    ...
                    /* Resolve Raduis */
                    var emailNotificationsOn =
                      emailNotificationsRadio.current.children[0].children[0];
                    updatedRecallSettings["email_notifications"] =
                      emailNotificationsOn.checked == true ? 1 : 0;

                    var imagesOn = imagesRadio.current.children[0].children[0];
                    updatedRecallSettings["image_enabled"] =
                      imagesOn.checked == true ? 1 : 0;

                    var audioOn = audioRadio.current.children[0].children[0];
                    updatedRecallSettings["audio_enabled"] =
                      audioOn.checked == true ? 1 : 0;

                    var smartCompletionOn =
                      smartCompletionRadio.current.children[0].children[0];
                    updatedRecallSettings["smart_completion"] =
                      smartCompletionOn.checked == true ? 1 : 0;

                    update_recall_settings(
                      setid,
                      updatedRecallSettings,
                      (data) => {
                        if (data.success) {
      ...
      <div className="top-info">
        {setData == undefined ? (
          <PlaceholderRectangle
            ...
            width="80px"
            height="80px"
            radius="50%"></PlaceholderRectangle>
        ) : (
        ...
        <div className="set-info">
          ...
          {setData == undefined ? (
            ...
            <div
              ...
              data-effect="solid"
              data-tip={"You are " + recallPercentage + "% complete!"}>
              <animated.div
                className="prog-fill"
                style={animateProgressBar}></animated.div>
            </div>
          )}
        </div>
      </div>
      <div className="mid-cover">
        ...
        {setData == undefined ? (
          <div className="flashcard-cover">
            <PlaceholderRectangle
              className="recall-flashcard-cover"
              width="601px"
              height="340px"
              color="#6658E8"
              radius="30px"
          ...
          <animated.div
            className="flashcard-cover"
            ref={confetti}
            style={showFlashcard}>
            <div className="front">
              {recallQueue.length > 0 &&
                recallSettings.image_enabled == true &&
                setData.terms != undefined &&
                setData.terms[recallQueue[currentTermPositionInQueue]] !=
                  undefined &&
                setData.terms[recallQueue[currentTermPositionInQueue]][2] !=
                  undefined && (
                  <animated.img
                    style={animateImage}
                    className="term-img"
              ...
              {recallQueue.length > 0 &&
                ...
                setData.terms[recallQueue[currentTermPositionInQueue]] !=
                  undefined &&
                !setData.terms[recallQueue[currentTermPositionInQueue]][4] && (
                  <animated.h1 style={animateTerm}>
                    {setData.terms[recallQueue[currentTermPositionInQueue]][0]}
                  </animated.h1>
                )}
              {recallQueue.length > 0 &&
                setData.terms != undefined &&
                ...
                setData.terms[recallQueue[currentTermPositionInQueue]][4] && (
                  ...
                  <animated.div style={animateTerm}>
                    <StaticMathField>
                      {
                        setData.terms[
                          recallQueue[currentTermPositionInQueue]
                        ][0]
                      }
                    </StaticMathField>
                  </animated.div>
              ...
              <animated.h2 style={animateSubtitle}>
                {recallSessionFinished
                  ? "You're on track to get that A! Come back " +
                    nextSession +
                    "."
                  : setData?.title +
                    " - " +
                    (currentTermPositionInQueue + 1) +
                    "/" +
                    recallQueue?.length}
              </animated.h2>
              {recallQueue?.length > 0 &&
              ...
              setData.terms[recallQueue[currentTermPositionInQueue]] !=
                undefined &&
              setData?.terms[recallQueue[currentTermPositionInQueue]][5] ? (
                <animated.div className="definition" style={animateDefinition}>
                  <StaticMathField>
                    {setData.terms[recallQueue[currentTermPositionInQueue]][1]}
                  </StaticMathField>
                </animated.div>
              ) : (
                recallQueue?.length > 0 &&
                ...
                setData.terms[recallQueue[currentTermPositionInQueue]] !=
                  undefined && (
                  <animated.a className="definition" style={animateDefinition}>
                    {recallQueue.length > 0 &&
                      setData.terms[recallQueue[currentTermPositionInQueue]][1]}
                  </animated.a>
                )
            ...
            {recallQueue.length > 0 &&
              ...
              !setData.terms[recallQueue[currentTermPositionInQueue]][5] && (
                <animated.div style={animateBottomFlashcard} className="back">
                  <animated.textarea
                    ref={bottomFlashcardInput}
                    style={animateBottomFlashcardTextArea}
                    placeholder={
                      recallQueue.length > 0 &&
                      !setData.terms[recallQueue[currentTermPositionInQueue]][1]
                        ? "Write as much as you know about the term '" +
                          setData.terms[
                            recallQueue[currentTermPositionInQueue]
                          ][0] +
                          "' here.."
                        : "Write as much as you know about the term here.."
                    }
                    onChange={function () {
                      setAnsweredDefinition(bottomFlashcardInput.current.value);
                    }}></animated.textarea>
                  <svg
                    ...
                    className="done-btn"
                    style={
                      bottomFlashcardInputDisabled
                        ? { visibility: "hidden" }
                        : {}
                    }
                    onClick={() => {
                      onFinishedTypingDefinition(answeredDefinition);
                    }}>
                    <path
                      id="Path_132"
                      data-name="Path 132"
                      d="M0,0H31.354V31.354H0Z"
                      fill="none"
                    />
                    <path
                      id="Path_133"
                      data-name="Path 133"
                      d="M10.716,19.448,5.229,13.961,3.4,15.79l7.316,7.316L26.393,7.429,24.564,5.6Z"
                      transform="translate(1.042 1.716)"
                      fill="#1e1e1e"
                    />
                  </svg>
                </animated.div>
              ...
              setData.terms[recallQueue[currentTermPositionInQueue]][5] && (
                <animated.div style={animateBottomFlashcard} className="back">
                  {/*<EditableMathField
                    ...
                    latex={answeredDefinition}
                    onChange={(mathField) => {
                      setAnsweredDefinition(mathField.latex());
                    }}
                  />*/}
                  {console.log(answeredDefinition)}
                  <EquationEditor
                    key={equationEditorKey}
                    value={answeredDefinition}
                    readOnly={equationEditorIsReadOnly}
                    onChange={setAnsweredDefinition}
            ...
            <animated.img
              className="emoji-stamp"
              style={animateEmojiStamp}
              src={currentTermStamp}></animated.img>
          </animated.div>
        )}
        <div className="smiley-faces-cover">
          <animated.div style={animateEmojis} className="smiley-faces">
            {setData != undefined && <h2>How well do you know this term?</h2>}

            <div className="smileys">
              {recallIntervals?.length > 0 &&
                recallIntervals?.map((str, index) => (
                  <div
                    className="smiley-cover"
                    onClick={function () {
