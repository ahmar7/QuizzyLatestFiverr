/**
 * Quizzy React JS Component
 * Property of Quizzy (2023)
 *
 * This source code is the property of Quizzy. Unauthorized use,
 * reproduction, or distribution of this code, or any portion of it,
 * is strictly prohibited without prior written permission from Quizzy.
 *
 * Quizzy. reserves the right to take legal action against any
 * individual or entity found to be in violation of these terms.
 */
...
const QuizExam = () => {
  ...
  // Viewing states
  const [isOnMainViewingResultsPage, setIsOnMainViewingResultsPage] = useState(false);
  const [isViewingDetails, setIsViewingDetails] = useState(false);
  const [viewingResults, setViewingResults] = useState(id ? true : false);
  const [quizView, setquizView] = useState(false);

  // Quiz states
  const [preparedAnswerForSubmission, setPreparedAnswerForSubmission] = useState(null);
  ...
  // Set data and time-related states
  ...
  if (quizDetails != null) {
    ...
    // most missed terms - calculate highest missed term index 
    for (var key in quizDetails[2]) {
      if (quizDetails[2][key] > highestMissedTermIndex) {
        highestMissedTermIndex = quizDetails[2][key]; 
      }
  ...
  function getMostMissedColor(percent) {
    var t = colorInterpolate("rgb(232, 165, 88)", "rgb(216, 60, 60)", percent);
    return "rgb(" + t["r"] + "," + t["g"] + "," + t["b"] + ")";
  }
  ...
  const [checking, setchecking] = useState(nQuestions < 3 ? false : true);
  const [mcqCheck, setmcqCheck] = useState(nQuestions < 5 ? false : true);
  useEffect(() => {
    // Autosave answers
    ...
    if (settingsVisible == true) {
      let bodys = document.querySelector("html");
  ...
  const [message, setMessage] = useState(getCurrentQuestionData().answer);
  ...
  return (
    ...
    >
      ...
      {settingsVisible && !isTakingQuiz && (
        <div id="opt" className="recall-session-option">
          ...
          <div className="recall-session-option-main">
            <div className="container">
              ...
              <div className="container-center vbg">
                  ...
                  <div className="quiz-details">
                    ...
                    <div className="most-missed">
                      ...
                      <div className="chart"> 
                        ...
                        >
                          {Object.keys(quizDetails[2]).map((key, c) => (
                            ...
                            <div className="label">
                              ...
                              <div className="chart-part" style={{
                                width: (quizDetails[2][key] / Object.keys(quizDetails[2]).length * 100) + "%",//(missed / total 
                                backgroundColor: getMostMissedColor(quizDetails[2][key] / highestMissedTermIndex),
                              }}></div>
                              {<div className="missed-number">
                                <h5 className="n-times" style={{color: getMostMissedColor((quizDetails[2][key] / highestMissedTermIndex))}}>
                                  {quizDetails[2][key] + " time" + (quizDetails[2][key] > 1 ? "s" : "") + ": "}
                                </h5>
                                {setData.terms[key][5] == null ? (
                                <h5 className="missed-definition">{setData.terms[key][1]}</h5>
                              ) : (
                                <StaticMathField>
                                  {setData.terms[key][1]}
                                </StaticMathField>
