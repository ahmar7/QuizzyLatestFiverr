...
function AddTask(props) {
  ...
  const evaluateSetSuggestions = function (str, recallNeeded = false) {
    ...
    function pushSuggestion(potentialSuggestion) {
      // ensure no duplicate suggestions
      var validSuggestion = true;
      if (
        recallNeeded &&
        potentialSuggestion.psd.has_recalled_before == false
      ) {
        validSuggestion = false;
        return false;
      }
      suggestions.forEach((suggestion) => {
        if (suggestion.id == potentialSuggestion.id) {
          // already suggested
          validSuggestion = false;
        }
      });
      addedSets.forEach((set) => {
        if (set.id == potentialSuggestion.id) {
          // set was already added
          validSuggestion = false;
        }
      });
        ...
        <div className="add-task-main">
          ...
          <div className="content">
            {stage == 0 && (
              <>
                ...
                <div className="task-list">
                  ...
                  <div
                    className={
                      (selectedTaskIndex == 1 ? "selected " : "") +
                      "task recall-sessions"
                    }
                    onClick={() => {
                  ...
                  <div
                    className={
                      (selectedTaskIndex == 2 ? "selected " : "") + "task quiz"
                    }
                    onClick={() => {
              ...
              <>
                ...
                >
                  <div
                    className={
                      "continue-btn" +
                      (addedSets.length == 0 ? " continue-btn-disabled" : "")
                    }
                  >
                    <p>Continue</p>
                  </div>
                ...
                <div
                  onClick={() => {
                    if (selectedTaskIndex == 0) {
                      // Flashcards - Ready to finish
                      add_task(
                        ["Flashcards", "Recall Session", "Quiz"][
                          selectedTaskIndex
                        ],
                        addedSets,
                        addedDates,
                  ...
                  className={
                    ...
                    (addedDates.length == 0 ? " continue-btn-disabled" : "")
                  }
                >
            ...
            {stage >= 3 && (
              <>
                <p>
                  {"Select the terms you want to practice for " +
                    addedSets[stage - 3].title}
                </p>
                ...
                {termChoiceIndex == 2 && (
                  <div
                    className="term-choices custom-choices"
                    ref={customChoice}
                  >
